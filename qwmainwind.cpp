...
//工具栏
    ...
    QAction *actMyAbout = new QAction("关于", this);
    // 给它设置一个系统自带的小图标
    actMyAbout->setIcon(style()->standardIcon(QStyle::SP_MessageBoxInformation));

    // 2. 把它加到工具栏上（放在最前面）
    // 注意：你原来的代码里有 addWidget，我们这里用 insertAction 把它插到最前面
    ui->mainToolBar->insertAction(ui->mainToolBar->actions().first(), actMyAbout);
    // 或者简单点直接加到最后也行：ui->mainToolBar->addAction(actMyAbout);

    // 3. 关联点击事件
    connect(actMyAbout, &QAction::triggered, this, &QWMainWind::showMyAboutDialog);
...
void QWMainWind::showMyAboutDialog()
{
    // 这里改成你自己的真实姓名和学号
    QString strName = "符烨茗";
    QString strID = "2024414290308";

    QString info = "开发人员信息：" + strName + "\n学号：" + strID;

    // 弹出对话框
    QMessageBox::information(this, "samp2_4", info, QMessageBox::Ok);
}
